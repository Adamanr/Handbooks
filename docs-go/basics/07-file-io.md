...
> <br /> **stdin *(сокращение от standard input)*** — это стандартный поток ввода в программе. Это место, откуда программа по умолчанию читает данные (текст), когда запускается.
...
**JSON *(JavaScript Object Notation)*** — это самый популярный формат для обмена данными: между сервером и клиентом, для хранения настроек, API и т.д.   В Go работа с JSON встроена в стандартную библиотеку — пакет [**`encoding/json`**](https://pkg.go.dev/encoding/json).
...
В Go работа с CSV встроена в стандартную библиотеку — пакет [**`encoding/csv`**](https://pkg.go.dev/encoding/csv).
