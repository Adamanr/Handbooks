---
sidebar_position: 2
description: "В этом уроке мы изучим основы синтаксиса Go, переменные, типы данных, операторы, указатели и комментарии."
---

# Основы синтаксиса

Добро пожаловать во второй урок! Теперь, когда вы установили Go и написали свою первую программу, пора разобраться, как "говорит" этот язык. Мы пройдёмся по основным строительным блокам: переменным, типам данных, операторам, указателям и комментариям.

Всё будет с простыми аналогиями из жизни, множеством примеров и мини-заданиями, чтобы вы сразу пробовали код сами. Рекомендую открыть редактор ([**VS Code**](https://code.visualstudio.com) + [**Go extension**](https://marketplace.visualstudio.com/items?itemName=golang.Go)) и повторять примеры по ходу чтения — так материал запомнится лучше.
...
Unicode отвечает только на вопрос: **какой номер у символа?**  
Он **не говорит**, как эти номера хранить в памяти или на диске.
...
**Unicode** — это огромный каталог всех символов мира с уникальными номерами.  
**UTF-8** — умный и экономный способ сохранить эти символы в байтах, который стал стандартом современного мира.
...
**Проблема с byte**:  
Многие символы (русские буквы, эмодзи, китайские иероглифы) не помещаются в один байт. Они кодируются в UTF-8 несколькими байтами (от 2 до 4).
...
```
Буква «П» занимает 2 байта, а эмодзи 👋 — 4 байта.  
...
### Число → строка
Для этого используем пакет [**`strconv`**](https://pkg.go.dev/strconv) (standard convert).
...
- `&x` — оператор "взять адрес" → возвращает указатель на `x`.
...
func main() {
    ...
    fmt.Println("x:", x)        // 10
    fmt.Println("p:", p)        // 0xc0000140a8 (адрес в памяти)
    fmt.Println("*p:", *p)      // 10 (значение по адресу)

    *p = 20                     // меняем значение по адресу
    fmt.Println("x теперь:", x) // 20 — x тоже изменился!
}
```
...
1. **New — стандартный способ создания**  
   ```go
   func NewUser(name string) *User {
       return &User{Name: name, CreatedAt: time.Now()}
   }
   ```
   Возвращаем указатель — так принято для структур.

2. **Слайсы и мапы — уже "ссылочные"**  
   Слайсы, мапы, каналы — содержат внутри указатель, поэтому передаются "по ссылке" автоматически.  
   Менять их внутри функции можно без `*`.

3. **Ошибки**  
   ```go
   func parseInt(s string) (*int, error) {
       // возвращаем указатель, если нужно "опциональное" значение
   }
   ```

    > Опциональное значение — это значение, которое может быть (присутствовать), а может и отсутствовать.
...
Все задачи решаются в одном файле `main.go` с хардкодом значений (без ввода с клавиатуры, кроме четвёртой задачи, где ввод опционален). Проверяй с помощью `go run main.go`.

### 1. **Площадь прямоугольника**  
   Задача: вычислить площадь прямоугольника по заданным сторонам.  

   Требования:  
   - Объявите переменные `length` и `width` типа `float64`.  
   - Задайте им осмысленные значения (например, 5.5 и 3.2).  
   - Вычислите площадь по формуле `S = length * width`.  
   - Выведите результат в формате: «Площадь прямоугольника со сторонами 5.5 и 3.2 равна 17.6».  

   Дополнительно: добавьте проверку, что значения сторон положительные. Если хотя бы одна сторона ≤ 0, выведите сообщение «Ошибка: стороны должны быть положительными».

### 2. **Обмен значениями**  
   Задача: поменять значения двух переменных местами без использования третьей переменной.  

   Требования:  
   - Создайте переменные: `a := 10`, `b := 20`.  
   - Поменяйте их значения местами, используя арифметические операции (сложение и вычитание).  
   - Выведите исходные и конечные значения в формате:  
     ```
     До обмена: a = 10, b = 20
     После обмена: a = 20, b = 10
     ```

   Дополнительно: реализуйте второй способ обмена через побитовую операцию XOR (^). Выведите результаты для обоих методов рядом для сравнения.

### 3. **Простой калькулятор**  
   Задача: создать калькулятор для двух чисел.  

   Требования:  
   - Объявите переменные `num1` и `num2` типа `float64` (например, 15.0 и 3.0).  
   - Выполните и выведите результаты четырёх операций: сложение, вычитание, умножение, деление.  
   - Для каждой операции выведите пояснение: «15.0 + 3.0 = 18.0» и т.д.  

   Дополнительно:  
   - Добавьте обработку случая, когда `num2 == 0` при делении (выведите: «Ошибка: деление на ноль!»).  
   - Форматируйте вывод с двумя знаками после запятой (`%.2f`).

### 4. **Конвертер температуры**  
   Задача: перевести температуру из шкалы Цельсия в шкалу Фаренгейта и обратно.  

   Требования:  
   - Объявите переменную `celsius` типа `float64` (например, 25.0).  
   - Вычислите значение в Фаренгейтах по формуле `F = celsius*9/5 + 32`.  
   - Выведите результат в формате: «25.0°C = 77.0°F».  
   - Реализуйте обратный перевод: задайте `fahrenheit := 77.0` и вычислите обратно в Цельсии по формуле `C = (fahrenheit - 32) * 5 / 9`.  

   Дополнительно: добавьте красивый вывод с эмодзи 🌡️ и проверку на экстремальные значения (например, ниже -100°C или выше 100°C — вывести предупреждение).

### 5. **Указатели: изменение значения через функцию**  
   Задача: понять, как указатели позволяют менять переменные внутри функции.  

   Требования:  
   - Создайте переменную `value := 10`.  
   - Напишите функцию `increment(val *int)`, которая увеличивает переданное значение на 5 (`*val += 5`).  
   - В `main` выведите значение до и после вызова функции:  
     ```
     До: 10
     После: 15
     ```
   - Добавьте вторую функцию `reset(val *int)`, которая обнуляет значение (`*val = 0`).  
   - Продемонстрируйте её работу.

### 6. **Указатели: опциональное значение**  
   Задача: реализовать функцию, возвращающую опциональное значение через указатель.  

   Требования:  
   - Напишите функцию `parsePositiveInt(s string) *int`, которая:  
     - Если строка представляет положительное целое число (> 0) — возвращает указатель на это число.  
     - Иначе возвращает `nil`.  
   - Захардкодьте несколько строк: `"42"`, `"0"`, `"-5"`, `"abc"`.  
   - Для каждой строки вызовите функцию и выведите:  
     - Если результат не `nil` → «Успешно: 42».  
     - Если `nil` → «Ошибка: недопустимое значение».  

   Пример вывода:  
   ```
   "42" → Успешно: 42
   "0"  → Ошибка: недопустимое значение
   "-5" → Ошибка: недопустимое значение
   "abc"→ Ошибка: недопустимое значение
   ```
